"""
CLI: plot multiple runs' TensorBoard bits-per-byte (val/bpb by default) on one
...
This is plot_eval_loss.py's twin, differing only in which scalar it reaches
for, and that difference is the point: val/loss is a per-TOKEN quantity and the
four arms tokenize differently, so their loss curves are not comparable across
arms. val/bpb (logged by every arm's train.py) divides total val bits by the
CLEAN val slice's byte count -- the same denominator everywhere -- which is
what actually puts the curves on one axis. Caveat carried over from the
training code: the bracket/scrambled arms' val text still carries markup, so
their bpb includes the cost of predicting that markup against the clean byte
count, and these are estimates over random eval windows, not the deterministic
full-pass numbers bpb_eval.py reports.
...
Usage:
    python plot_bpb.py                    # the 4 arms of the split in split_config.py
    python plot_bpb.py --split 43         # the 4 arms of split 43
    python plot_bpb.py bracket control    # only these two
    python plot_bpb.py split42/bracket split43/bracket   # same arm, two splits
    python plot_bpb.py --tag val/loss     # a different scalar tag
    python plot_bpb.py -o comparison.png  # save instead of showing
"""
import argparse
import os
import sys
...
def main():
    ...
    parser.add_argument('--tag', default='val/bpb', help="scalar tag to plot (default: val/bpb)")
    ...
    plt.ylabel(args.tag)
    plt.title(f'{args.tag} by run (split {args.split})')
    plt.legend()
