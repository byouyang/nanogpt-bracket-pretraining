...
Does NOT affect corpus construction (split_config.py's SPLIT_SEED, or
scramble_brackets.py's --seed) -- those control dataset splits/scrambling,
a separate concern from training-run variance.

Note this is NOT the axis the experiment currently varies, and it does not name
any output directory: SPLIT_SEED does (split<SPLIT_SEED>/<arm>). Holding SEED
fixed while varying SPLIT_SEED is the intended setup -- see split_config.py.
"""
SEED = 42
